    ...
    **Multi_GPU training**:  If the wrapped ``init_optimizer`` was created from a model wrapped in
    ...
    def __init__(self,
        ...
        for i, param_group in enumerate(self.optimizer.param_groups):
            self.maybe_print(
                "FP16_Optimizer processing param train_args {}:".format(i))
    ...
    def inspect_master_grad_data(self):
        ...
        Returns:
            List of lists (one list for each parameter train_args).  The list for each parameter train_args
            is a list of the ``.grad.data`` attributes of the fp32 master params belonging to that train_args.
        """
        if self.overflow:
            print(
