...
class LMDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.block_lm:
            ...
            num_special_tokens = num_special_tokens_to_add(prompt,
                                                           None,
                                                           text,
                                                           add_cls=True,
                                                           add_sep=False,
                                                           add_piece=True,
                                                           add_eos=False)
            data = build_input_from_ids(prompt,
                                        None,
                                        text,
                                        self.max_seq_len + num_special_tokens +
                                        1,
                                        self.tokenizer,
                                        args=self.args,
                                        add_cls=True,
            ...
            if len(tokens) < self.max_seq_len:
                ...
                loss_masks = loss_masks + [0] * (self.max_seq_len -
                                                 len(loss_masks))
            if idx != 0:
                loss_masks = np.array(loss_masks, dtype=np.int64)
                loss_masks[:-self.overalapping_eval] = 0
